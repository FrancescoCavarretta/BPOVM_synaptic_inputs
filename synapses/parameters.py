...
from bpo.ephys.serializer import DictMixin
...
class PyParameter(bpo.parameters.Parameter):
    """Contructor"""
    def __init__(self, name, value=None, frozen=False, bounds=None, py_param_name=None):
        super(PyParameter, self).__init__(name, value=value, frozen=frozen, bounds=bounds)
        self.py_param_name = py_param_name
...
class PyObjectParameter(PyParameter, DictMixin):
    ...
    def __init__(self, name, value=None, frozen=False, bounds=None, py_param_name=None, py_objects=None):

        super(PyParameter, self).__init__(name, value=value, frozen=frozen, bounds=bounds, py_param_name=py_param_name)

        self.py_objects = syn_groups
    ...
    def instantiate(self, sim=None, icell=None):
        ...
        for _py_object in self.py_objects:
          setattr(_py_object, self.py_param_name, self.value)
          ...
          logger.debug('Set %s in %s to %s', self.py_param_name, _py_object, self.value)
    ...
    def __str__(self):
        ...
        return '%s: %s %s = %s' % (self.name, [str(_py_object) for _py_object in self.py_objects], self.py_param_name, self.value if self.frozen else self.bounds)
